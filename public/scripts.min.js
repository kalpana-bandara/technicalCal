...
loadCoin()})
async function generateQuote(){const quoteResponse=await fetch("https://api.quotable.io/random?tags=technology&minLength=70&maxLength=160")
...
document.getElementById("form").insertAdjacentHTML('beforeend',`
  ...
  `)}
async function loadCoin(){if(coinName.value==''){alert("Please Enter a Coin Name")
location.reload()}else{const response=await fetch(`https://shrouded-bayou-23618.herokuapp.com/https://api.taapi.io/fibonacciretracement?secret=${secret}&exchange=binance&symbol=${coinName.value}/USDT&interval=${time.value}`)
...
if(data.errors){alert("Please Enter Valid Coin Name")
location.reload()}else{createTable(data,dataTwo,dataThree,dataFour,dataFive,macdData,emaData)}}
document.getElementById("first").style.display="none"
document.getElementById("details-cards").style.display="block"}
function createTable(x,b,c,supertrend,price,macd,ema){let emaValue
if(price.value<ema.value){emaValue="Price below 200EMA"}else if(price.value>ema.value){emaValue="Price above 200EMA"}
let cross
if(macd.valueMACD>0&&macd.valueMACDSignal>0){if(macd.valueMACD>macd.valueMACDSignal){cross="Crossed Upwards"}else if(macd.valueMACD<macd.valueMACDSignal){cross="Crossed Downwards"}}else if(macd.valueMACD<0&&macd.valueMACDSignal<0){if(macd.valueMACD<macd.valueMACDSignal){cross="Crossed Downwards"}else if(macd.valueMACD>macd.valueMACDSignal){cross="Crossed Upwards"}}else{cross="Neutral"}
let guess
if(c.value>=70){guess="Oversold!"}else if(c.value<=30){guess="Overbought!"}else if(c.value>50&&c.value<56){guess="Possible Downtrend"}else if(c.value<50&&c.value>45){guess="Possible Uptrend"}else if(c.value<45){guess="Downtrending"}else if(c.value>55){guess="uptrending"}
if(b.value==0){b.value=`No `}else if(b.value==1){b.value=`Yes`}
let bullOrBear
if(x.value<price.value){bullOrBear=`<li style = "color:green;"><b>Price is  Above 0.618</b></li>`}else{bullOrBear=`<li style = "color:red;"><b>Price is  Below 0.618</b></li>`}
...
<div class="coin">
  ...
  <p style="cursor:pointer" onClick="location.reload()">Analyze another coin</p>
    ...
    `
if(guess=="Possible Uptrend"){document.getElementById("rsi").style.color="Green"}else if(guess=="Possible Downtrend"){document.getElementById("rsi").style.color="Red"}
let pivotButton=document.getElementById("pivotButton")
pivotButton.addEventListener("click",loadPivot)}
async function loadPivot(){const responseSeven=await fetch(`https://shrouded-bayou-23618.herokuapp.com/https://api.taapi.io/candle?secret=${secret}&exchange=binance&symbol=${coinName.value}/USDT&interval=1d&backtrack=1`)
...
function displayCandleDetails(candleprev,cPrice){let pivotShow=document.querySelector(".pivotShow")
...
pivotShow.innerHTML=`
           ...
           `
let opinion
if(cPrice.value>pp){opinion=`<b>Current price is above Pivot Point!</b>`}else if(cPrice.value<pp){opinion=`<b>Current price is below Pivot Point!</b>`}else if(cPrice.value>r1){opinion=`<b>Current price is above Resistance 1!</b>`}else if(cPrice.value<s1){opinion=`<b>Current price is below Support 1!</b>`}
